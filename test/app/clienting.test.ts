...
import { AuthMode, SignifyClient } from '../../src/keri/app/clienting';
...
import { Signage, signature } from '../../src/keri/end/ending';
import {
    EssrAuthenticator,
    SignedHeaderAuthenticator,
} from '../../src/keri/core/authing';
...
describe('SignifyClient', () => {
    test('SignifyClient initialization', async () => {
        ...
        assert.throws(
            ...
            { message: 'bran must be 21 characters' }
        );
        ...
        assert.equal(client.agent!.state.s, '0');
        assert.equal(
    ...
    test('Create signed headers HTTP request', async () => {
        ...
        assert.equal(lastBody.foo, true);
    ...
    test('Client authenticated fetch', async () => {
        ...
        assert.equal(prepareSpy.mock.calls.length > 0, true);
        assert.equal(
            prepareSpy.mock.calls[0][1],
            'ELI7pg979AdhmvrjDeam2eAO2SR5niCgnjAJXJHtJose'
        );
        assert.equal(
            prepareSpy.mock.calls[0][2],
            'EEXekkGu9IAzav6pZVJhkLnjtjM5v3AcyA-pdKUcaGei'
        );
        let request = prepareSpy.mock.calls[0][0];

        assert.equal(request.url, 'http://127.0.0.1:3901/identifiers');
        assert.equal(request.method, 'GET');
        assert.equal(
            request.headers.get(HEADER_SIG_SENDER),
            'ELI7pg979AdhmvrjDeam2eAO2SR5niCgnjAJXJHtJose'
        );
        assert.equal(request.headers.has(HEADER_SIG_TIME), true);
        assert.equal(request.headers.has('Content-Type'), false);
        assert.equal(request.headers.has('Content-Length'), false);
        assert.equal(request.body, null);

        assert.equal(verifySpy.mock.calls.length > 0, true);
        assert(verifySpy.mock.calls[0][0] instanceof Request);
        assert(verifySpy.mock.calls[0][1] instanceof Response);
        assert.equal(verifySpy.mock.calls[0][0], request); // Pass baseRequest, not wrapped request (for case of ESSR)
        assert.equal(verifySpy.mock.calls[0][1], getResponse);
        ...
        assert.equal(request.url, 'http://127.0.0.1:3901/oobis');
        ...
        assert.equal(request.headers.has(HEADER_SIG_TIME), true);
        assert.equal(request.headers.get('Content-Type'), 'application/json');
        assert.equal(request.headers.get('A'), '1');
        assert.equal(request.headers.get('B'), '2');
        assert.deepEqual(await request.json(), {
            url: 'http://localhost:5642/oobi/BBilc4-L3tFUnfM_wJr4S4OJanAv_VmF_dJNN6vkf2Ha',
            alias: 'wit',
        });
