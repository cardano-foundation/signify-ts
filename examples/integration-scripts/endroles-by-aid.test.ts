...
describe('endroles-by-aid', () => {
    test('alice queries /endroles/{aid}', async () => {
        const result = await aliceClient.oobis().endroles(groupAid);
        expect(result).not.toHaveLength(0);
        expect(result.every((r: any) => r.cid === groupAid)).toBe(true);
        expect(result.every((r: any) => r.role === 'agent')).toBe(true);
        const eids = result.map((r: any) => r.eid).sort();
        expect(eids).toEqual([...agentEids].sort());
    });

    test('alice queries /endroles/{aid}/agent', async () => {
        const result = await aliceClient.oobis().endroles(groupAid, 'agent');
        expect(result).not.toHaveLength(0);
        expect(result.every((r: any) => r.role === 'agent')).toBe(true);
        expect(result.every((r: any) => r.cid === groupAid)).toBe(true);
        const eids = result.map((r: any) => r.eid).sort();
    ...
    test('alice queries non-existent role returns empty', async () => {
        const result = await aliceClient.oobis().endroles(groupAid, 'mailbox');
    ...
    test('alice queries her own AID returns her agent eid', async () => {
        ...
        const result = await aliceClient.oobis().endroles(aliceAid);
