...
Actors:
- **M1**: group member 1 (initiator — receives `/wap/iss`, builds VCP+ISS, sends ACK)
- **M2**: group member 2 (cosigner — co-signs VCP+ISS via correlationId)
...
### Test
- `examples/integration-scripts/wap-group-issuance.test.ts` — the test

### Setup scripts (run before the test)
...
1. Loads the four clients from `.test-clients.json` (boots agents if KERIA was wiped between setup and test — shouldn't normally happen)
2. Loads identifiers and the group from existing KERIA state
3. Sanity-checks that contacts are mutually resolved (fails fast if setup didn't run)
4. Drives the flow:
   - CS sends `/wap/iss` to G1
   - M1 receives `/exn/wap/iss`, creates VCP, issues credentials, sends `/multisig/vcp` + `/multisig/iss` + `/multisig/exn` (ACK wrapper)
   - M2 polls notifications for `/multisig/vcp` and `/multisig/iss` matching the `correlationId`, co-signs each
   - M2 sends `/multisig/exn` (ACK wrapper, same `dt` as M1's → deterministic SAID)
   - KERIA counselor assembles both signatures → delivers `/exn/wap/iss/ack` to CS
5. Asserts CS receives the ACK notification
