...
export class Credentials {
     ...
     * @param {Serder} acdc ACDC to process and verify
     * @param {Serder} iss Issuing event for ACDC in TEL
     * @returns 
     */
    async verify(acdc: Serder, iss: Serder) {
...
export class Registries {
    ...
    async get(ri: string): Promise<any> {
        const path = `/registries/${ri}`;
        const method = 'GET';
        const res = await this.client.fetch(path, method, null);
        return res.json();
    }
